...
const serverDir = path.join(
  __dirname,
  "../../../..",
  configJSON.Bot_Config.Servers_Config.servers_path,
);
...
export class ChannelDeleter {
  public checkUsersOf(channel: VoiceChannel) {
    const configData = Deno.readDir(
      path.join(serverDir, channel.guild!.id, config),
    );
    const tempChannelsContainerID =
      configData.Vocals_Options.vocals_category_id;

    if (
      channel.isVoiceBased() && channel.parentId === tempChannelsContainerID
    ) {
  ...
  public checkUsersBulk(guild: GuildManager) {
    guild!.cache.forEach(async (g) => {
      const config = await new DataWriter().read(
        path.join(serverDir, g.id, config),
      );
      const tempChannelsContainerID = config.Vocals_Options.vocals_category_id;
      const tempChannel = g.channels.cache.filter((c) =>
        c.isVoiceBased() && c.parentId === tempChannelsContainerID
      );
  ...
  private async deleteTempChannel(target: VoiceChannel) {
    ...
    const allowDeletion =
      config.Vocals_Options.purge_options.purge_empty_channels;
    ...
    invites.each((invite) => {
      const inviteChannel = invite.guild.channels.cache.get(
