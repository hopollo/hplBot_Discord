...
export class ChannelCreator {
  ...
  async createNewVoiceChannel(msg: Message, channelName: string, slotsLimit: number) {
    ...
    const tempChannels = await import(path.join(__dirname, '../..', Bot_Config.Servers_Config.servers_path, msg.guild!.id, Bot_Config.Servers_Config.templates.tempChannelsFile));
    ...
    const allowLogs: boolean = config.Channels_Options.logs_options.channels_creations.enabled;
    ...
    const newChannel: VoiceChannel = await msg.guild?.channels.create(channelName, {
      type: 'voice',
    ...
    if (deleteCreationCommand) msg.delete({timeout: msgDeleteIdle});

    if (shareInvite) {
      new CreateInvite(newChannel);
    } else {
      // Add both to use them for deletions after
      new DataWriter().appendTo(tempChannels, {channelID: newChannel.id, inviteID: null});
    }
